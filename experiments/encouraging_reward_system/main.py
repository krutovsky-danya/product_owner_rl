...
from environment.reward_system.EncouragingRewardSystem import EncouragingRewardSystem
...
def make_credit_study(trajectory_max_len, episode_n) -> LoggingStudy:
    reward_system = EncouragingRewardSystem(config={})
...
def main():
    ...
    pd.DataFrame(study.sprints_log).to_csv("sprints.csv", index=False)
    ...
    plt.grid()
    plt.legend()
    plt.savefig("encouraging_reward_system.png")
    plt.show()
